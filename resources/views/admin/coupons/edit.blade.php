...
<div class="wrap">
    <div class="content">
        <div style="border-bottom: 3px solid #F7B633; padding-bottom: 10px; margin-bottom: 30px;">
            <h1 style="text-transform: uppercase; color: #070D59; margin: 0;">Editar Cupón: {{ $coupon->code }}</h1>
        </div>
        ...
        <div style="background: white; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); padding: 30px; max-width: 600px; margin: 0 auto;">
            <form action="{{ route('admin.coupons.update', $coupon->id) }}" method="POST">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="text" name="code" value="{{ old('code', $coupon->code) }}" required style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 5px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="value" value="{{ old('value', $coupon->value) }}" required style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 5px;">
                        @error('value') <small style="color: #e74c3c;">{{ $message }}</small> @enderror
                    </div>
                </div>

                <div style="margin-bottom: 20px;">
                    <label style="display: block; font-weight: bold; margin-bottom: 8px; color: #555;">Fecha de Expiración</label>
                    <input type="date" name="expires_at" value="{{ old('expires_at', $coupon->expires_at ? $coupon->expires_at->format('Y-m-d') : '') }}" style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 5px;">
                    @error('expires_at') <small style="color: #e74c3c;">{{ $message }}</small> @enderror
                </div>

                <div style="margin-bottom: 30px;">
                    <label style="display: flex; align-items: center; gap: 10px; cursor: pointer;">
                        <input type="checkbox" name="is_active" value="1" {{ $coupon->is_active ? 'checked' : '' }} style="width: 18px; height: 18px;">
